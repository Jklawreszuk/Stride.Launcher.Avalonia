...
{
    public class DocumentationPageViewModel : ReactiveObject
    {
        private static readonly Regex ParsingRegex = new(@"\{([^\{\}]+)\}\{([^\{\}]+)\}\{([^\{\}]+)\}");
        ...
        public DocumentationPageViewModel(string version)
        {
            Version = version;
            OpenUrlCommand = ReactiveCommand.Create<string>(OpenUrl);
        }
        // Code duplication, need to move somewhere
        ...
        public static async Task<List<DocumentationPageViewModel>> FetchGettingStartedPages(string version)
        {
                ...
                if (urlData != null)
                {
                    ...
                    foreach (var url in urls)
                    {
                        ...
                        if (match.Success && match.Groups.Count == 4)
                        {
                            ...
                            var page = new DocumentationPageViewModel(version)
                            {
